 ...
 *
 * $Id: wcte11xp.c,v 1.4 2004/12/13 19:31:41 markster Exp $
 */
...
#define WCDELAY	0x0	/* 30 = 15 cycles, 10 = 8 cycles, 0 = 3 cycles */
...
static int debug = 2;   /* doesnt do anything */
static int alarmdebounce = 0;
static int loopback = 0;
static int clockextra = 0;
static int t1e1override = 0; // -1;
...
#ifdef SOLARIS
	if (wc->dma_acc_handle)
        	ddi_dma_mem_free(&wc->dma_acc_handle);
	if (wc->dma_handle)
...
static void t1xxp_start_dma(struct t1 *wc)
{
	...
	outb(WCDELAY | 0x0f, wc->ioaddr + WC_CNTL);
	set_current_state(TASK_INTERRUPTIBLE);
	schedule_timeout(1);
	outb(WCDELAY | 0x01, wc->ioaddr + WC_CNTL);
...
#ifdef SOLARIS
static int t1xxp_ioctl(struct zt_chan *chan, unsigned int cmd, unsigned long data, int mode)
#else
...
static int t1xxp_startup(struct zt_span *span)
{
	...
	if (!alreadyrunning) {
		...
		cmn_err(CE_CONT, "enabled interrupts.\n");
		t1xxp_start_dma(wc);
		cmn_err(CE_CONT, "started dma.\n");
		span->flags |= ZT_FLAG_RUNNING;
		cmn_err(CE_CONT, "set zt_flag_running.\n");
	}
	return 0;
}
...
static uint_t t1xxp_interrupt(caddr_t dev_id)
{
	struct t1 *wc = (struct t1 *)dev_id;
	unsigned char ints;
	unsigned long flags;
	int x;
...
static int t1xxp_hardware_init(struct t1 *wc)
{
	...
	outb(WCDELAY | 0x0e, wc->ioaddr + WC_CNTL);
...
static int t1xxp_init_one(struct t1 *wc)
{
	int res;
        size_t rlen;
	...
	unsigned int *canary;
		...
		if (wc) {
                        if (res=ddi_regs_map_setup(wc->dev, 1, &wc->ioaddr, 0, 255,
                                &dev_attr, &wc->devhandle) != DDI_SUCCESS) {
				cmn_err(CE_CONT, "wcte11xp: ddi_regs_map_setup failed.\n");
				if (res == DDI_FAILURE)
				cmn_err(CE_CONT, "returned failure\n");
				else if (res == DDI_ME_RNUMBER_RANGE)
				cmn_err(CE_CONT, "me_rnumber_range\n");
				else if (res == DDI_REGS_ACC_CONFLICT)
				cmn_err(CE_CONT, "regs_acc_conflict\n");
				else
				cmn_err(CE_CONT, "returned %d\n", res);
                                return DDI_FAILURE;
			}
			wc->offset = 28;	/* And you thought 42 was the answer */

			size = ZT_MAX_CHUNKSIZE * 32 * 2 * 2;
				...
				cmn_err(CE_CONT, "wcte11xp: ddi_dma_alloc_handle failed.\n");
                                return (DDI_FAILURE);
                         }
				...
				cmn_err(CE_CONT, "wcte11xp: ddi_dma_mem_alloc failed.\n");
                            ddi_dma_free_handle(&wc->dma_handle);
                                return (DDI_FAILURE);
                        }
                        ...
                        if (ddi_dma_addr_bind_handle(wc->dma_handle,
                            ...
                            &cookie,
                            &count) != DDI_SUCCESS) {
				cmn_err(CE_CONT, "wcte11xp: ddi_dma_addr_bind_handle failed.\n");
			...
			*canary = (CANARY << 16) | (0xffff);
                        ...
                        comm |= PCI_COMM_ME; /* enable dma */
                        comm &= ~PCI_COMM_PARITY_DETECT; /* disable parity error checking */
                        pci_config_put16(wc->pci_conf_handle, PCI_CONF_COMM, comm);

/*
			if (ddi_get_iblock_cookie(wc->dev, 0, &wc->iblock) != DDI_SUCCESS) {
				cmn_err(CE_CONT, "wctdm: Unable to ddi_get_iblock_cookie\n");
				(void) ddi_dma_mem_free(&wc->dma_acc_handle);
				ddi_dma_free_handle(&wc->dma_handle);
				return (DDI_FAILURE);
			}
			...
			if (ddi_intr_hilevel(wc->dev, 0) != 0) {
				cmn_err(CE_CONT, "wctdm: hilevel interrupts not supported");
			}

                        if (ddi_add_intr(wc->dev, 0, NULL, NULL,
                                        t1xxp_interrupt, (caddr_t)wc) != DDI_SUCCESS) {
...
*/
			...
			cmn_err(CE_CONT, "checking for High Level interrupt\n");
			if (ddi_intr_hilevel(wc->dev, 0) != 0) {
			...
			cmn_err(CE_CONT, "The number of interrupts %d\n", resultp);

			cmn_err(CE_CONT, "retrieve interrupt block cookie\n");
			resultp=ddi_get_iblock_cookie(wc->dev, 0, &wc->iblock);
			if (resultp != DDI_SUCCESS) {
				cmn_err(CE_CONT, "Interrupt not found\n");
				(void) ddi_dma_mem_free(&wc->dma_acc_handle);
				ddi_dma_free_handle(&wc->dma_handle);
				return (DDI_FAILURE);
			}
			cmn_err(CE_CONT, "got interrupt cookie.\n");

			/* init the mutex */
			spin_lock_init(&wc->lock);
			cmn_err(CE_CONT, "initialized mutex.\n");

			if (ddi_add_intr(wc->dev, 0, 0, 0, t1xxp_interrupt, (caddr_t)wc) != DDI_SUCCESS) {
				cmn_err(CE_CONT, "ddi_add_intr failed.\n");
				(void) ddi_dma_mem_free(&wc->dma_acc_handle);
				ddi_dma_free_handle(&wc->dma_handle);
				return (DDI_FAILURE);
			}
			cmn_err(CE_CONT, "interrupt handler installed.\n");
			res = 0;
		} else
			res = -ENOMEM;
#ifndef SOLARIS
...
#endif
	cmn_err(CE_CONT, "leaving t1xxp_init_one\n");
	return res;
}
...
#ifdef SOLARIS
static void t1xxp_remove_one(struct t1 *wc)
#else
...
#ifdef SOLARIS
static int t1xxp_getinfo(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
                    void **result);
static int t1xxp_attach(dev_info_t *dip, ddi_attach_cmd_t cmd);
static int t1xxp_detach(dev_info_t *dip, ddi_detach_cmd_t cmd);

/* "char/block operations" OS structure */
static struct cb_ops    t1xxp_cb_ops = {
    nulldev,                    /* open() */
    nulldev,                    /* close() */
    nodev,                      /* strategy()           */
    nodev,                      /* print routine        */
...
static struct dev_ops t1xxp_ops = {
    DEVO_REV,                   /* devo_rev */
    0,                          /* devo_refcnt */
    t1xxp_getinfo,              /* devo_getinfo */
    nulldev,                    /* devo_identify */
    nulldev,                    /* devo_probe */
    t1xxp_attach,               /* devo_attach */
    t1xxp_detach,               /* devo_detach */
    nodev,                      /* devo_reset */
    &t1xxp_cb_ops,              /* devo_cb_ops */
...
static  struct modldrv modldrv = {
    &mod_driverops,
    "Wildcard T100P/E100P Zaptel Driver",
    &t1xxp_ops,
};
...
int _init(void)
{
  ...
  ret = ddi_soft_state_init(&wcstatep, sizeof(struct t1), WC_MAX_CARDS);
  if (ret != 0)
  {
    return ret;
  }
  ...
  if ((ret = mod_install(&modlinkage)) != 0) {
    cmn_err(CE_CONT, "t1xxp: _init FAILED");
    return DDI_FAILURE;
  }
...
int _fini(void)
{
    ...
    if ((ret = mod_remove(&modlinkage)) == 0) {
        ddi_soft_state_fini(&wcstatep);
    }
    return ret;
}

static int t1xxp_getinfo(dev_info_t *dip, ddi_info_cmd_t infocmd,
              void *arg, void **result)
{
  ...
  switch (infocmd)
  {
    case DDI_INFO_DEVT2DEVINFO:
      ...
      state = ddi_get_soft_state(wcstatep, instance);
      if (state != NULL)
      {
        *result = state->dev;
        error = DDI_SUCCESS;
...
static int t1xxp_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
  ...
  cmn_err(CE_CONT, "t1xxp: Entering attach.");
  switch (cmd) {
  case DDI_RESUME:
    cmn_err(CE_CONT, "t1xxp: Ignoring attach_RESUME");
    return DDI_FAILURE;
  case DDI_PM_RESUME:
    cmn_err(CE_CONT, "t1xxp: Ignoring attach_PM_RESUME");
    return DDI_FAILURE;
  case DDI_ATTACH:
    break;
  default:
    cmn_err(CE_CONT, "t1xxp: unknown attach command %d", cmd);
    return DDI_FAILURE;
  }
  ...
  if (ddi_soft_state_zalloc(wcstatep, instance) != DDI_SUCCESS)
  {
    cmn_err(CE_CONT, "t1xxp%d: Failed to alloc soft state", instance);
    return DDI_FAILURE;
  }
  ...
  if (state == NULL)
  {
    cmn_err(CE_CONT, "t1xxp%d: attach, failed to get soft state", instance);
    ddi_soft_state_free(wcstatep, instance);
    return DDI_FAILURE;
  }
  ...
  if (t1xxp_init_one(state) != DDI_SUCCESS)
  {
    cmn_err(CE_CONT, "t1xxp%d: Failed to initialise device", instance);
    ddi_soft_state_free(wcstatep, instance);
    return DDI_FAILURE;
  }
...
static int t1xxp_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
    ...
    cmn_err(CE_CONT, "t1xxp: Entering detach.");

    instance = ddi_get_instance(dip);

    state = ddi_get_soft_state(wcstatep, instance);
    /* bzero(&state, sizeof(struct t1)); */
    if (state == NULL) {
        cmn_err(CE_CONT, "t1xxp%d: detach, failed to get soft state", instance);
        return DDI_FAILURE;
    }

    t1xxp_remove_one(state);
