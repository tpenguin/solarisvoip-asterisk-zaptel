...
static long wcfxo_irqcount = 0;

static void *wcfxostatep;
...
#define outb(b,a)	ddi_io_put8(wc->devhandle, a, b)
#define inl(a)		ddi_io_get32(wc->devhandle, (unsigned int *)a)
// #define outl(b,a)	ddi_io_put32(wc->devhandle, (unsigned int *)a, (unsigned int) b)
...
#define cpu_to_le32(a)	(((a>>24) & 255) | (((a>>16) & 255) <<8) | \
			 (((a>>8) & 255) <<16) | ((a & 255) <<24))
...
static inline void wcfxo_transmitprep(struct wcfxo *wc, unsigned char ints)
{
	...
	bcopy(wc->chan.writechunk, wc->lasttx, ZT_CHUNKSIZE);
	...
	for (x=0;x<ZT_CHUNKSIZE;x++) {
		...
		case FLAG_WRITE:
			...
			break;
		default:
			cmn_err(CE_CONT, "wcfxo: Huh?  No read or write??\n");
			cmd = 0;
		}
	...
	for (x=0;x<ZT_CHUNKSIZE;x++) {
		...
		if (wc->oldregs[x].flags == FLAG_READ && !wc->ignoreread) {
			...
			if ((realval == 0x89) && (realreg != 0x9)) {
				...
				if (debug)
					cmn_err(CE_CONT, "New regoffset: %d\n", wc->regoffset);
			}
...
static uint_t wcfxo_interrupt(caddr_t arg)
{
	struct wcfxo *wc = (struct wcfxo *)arg;
	unsigned char ints;
	unsigned char b;
#ifdef DEBUG_RING
	static int oldb = 0;
	...
	mutex_enter(&wc->devlock);

	if (!wc->devhandle) {
		/* We're not setup yet! */
		mutex_exit(&wc->devlock);
		return DDI_INTR_UNCLAIMED;
	}

	ints = inb(wc->ioaddr + WC_INTSTAT);
	outb(ints, wc->ioaddr + WC_INTSTAT);
cmn_err(CE_NOTE, "ints = %x\n", ints);
	...
	if (ints & 0x10) {
		cmn_err(CE_CONT, "FXO PCI Master abort\n");
		mutex_exit(&wc->devlock);
	...
	if (ints & 0x20) {
		cmn_err(CE_CONT, "PCI Target abort\n");
		mutex_exit(&wc->devlock);
		wcfxo_stop_dma(wc);
	...
	if (1 /* !(wc->report % 0xf) */) {
		...
		if (!b) {
...
#if 0
			if (wc->battery)
				cmn_err(CE_CONT, "Battery loss: %d (%d debounce)\n", b, wc->battdebounce);
#endif
			if (wc->battery && !wc->battdebounce) {
				if (debug)
					cmn_err(CE_CONT, "NO BATTERY!\n");
				wc->battery =  0;
#ifdef	JAPAN
				if ((!wc->ohdebounce) && wc->offhook) {
					zt_hooksig(&wc->chan, ZT_RXSIG_ONHOOK);
					if (debug)
						cmn_err(CE_CONT, "Signalled On Hook\n");
#ifdef	ZERO_BATT_RING
					wc->onhook++;
#endif
				}
			...
			if (!wc->battery && !wc->battdebounce) {
				if (debug)
					cmn_err(CE_CONT, "BATTERY!\n");
#ifdef	ZERO_BATT_RING
				if (wc->onhook) {
					wc->onhook = 0;
					zt_hooksig(&wc->chan, ZT_RXSIG_OFFHOOK);
					if (debug)
						cmn_err(CE_CONT, "Signalled Off Hook\n");
				}
#else
				zt_hooksig(&wc->chan, ZT_RXSIG_OFFHOOK);
#endif
...
static int wcfxo_setreg(struct wcfxo *wc, unsigned char reg, unsigned char value)
{
	...
	cmn_err(CE_CONT, "wcfxo: Out of space to write register %02x with %02x\n", reg, value);
	return -1;
}
...
{
	cmn_err(CE_CONT, "FXO: Restarting DMA\n");
...
static int wcfxo_hooksig(struct zt_chan *chan, zt_txsig_t txsig)
{
	...
	case ZT_TXSIG_ONHOOK:
		...
		break;
	default:
		cmn_err(CE_CONT, "wcfxo: Can't set tx state to %d\n", txsig);
	}
	if (debug)
		cmn_err(CE_CONT, "Setting hook state to %d (%02x)\n", txsig, reg);
	return 0;
}
...
static int wcfxo_initialize(struct wcfxo *wc)
{
	...
	cv_init(&wc->span.maintq, NULL, CV_DRIVER, NULL);

	wc->span.pvt = wc;
	wc->chan.pvt = wc;
	if (zt_register(&wc->span, 0)) {
		cmn_err(CE_CONT, "Unable to register span with zaptel\n");
		return -1;
	}
	return 0;
}
...
static int wcfxo_hardware_init(struct wcfxo *wc)
{
	...
	/* Don't want to get caught in interrupts... */
	mutex_enter(&wc->devlock);
...
cmn_err(CE_CONT, "About to write DMA addresses\n");
	/* Setup DMA Addresses */
cmn_err(CE_CONT, "DMAWS = %lx\n", wc->writedma);
	outl(wc->writedma,                    wc->ioaddr + WC_DMAWS);		/* Write start */
cmn_err(CE_CONT, "DMAWI = %lx\n", wc->writedma + ZT_CHUNKSIZE * 8 - 4);
	outl(wc->writedma + ZT_CHUNKSIZE * 8 - 4, wc->ioaddr + WC_DMAWI);		/* Middle (interrupt) */
cmn_err(CE_CONT, "DMAWE = %lx\n", wc->writedma + ZT_CHUNKSIZE * 16 - 4);
	outl(wc->writedma + ZT_CHUNKSIZE * 16 - 4, wc->ioaddr + WC_DMAWE);			/* End */
	
cmn_err(CE_CONT, "DMARS = %lx\n", wc->readdma);
	outl(wc->readdma,                    	 wc->ioaddr + WC_DMARS);	/* Read start */
cmn_err(CE_CONT, "DMARI = %lx\n", wc->readdma + ZT_CHUNKSIZE * 8 - 4);
	outl(wc->readdma + ZT_CHUNKSIZE * 8 - 4, 	 wc->ioaddr + WC_DMARI);	/* Middle (interrupt) */
cmn_err(CE_CONT, "DMARE = %lx\n", wc->readdma + ZT_CHUNKSIZE * 16 - 4);
	...
	for(x=0; x<0x30; x++)
	{
	   cmn_err(CE_CONT, "IO[%02x] = %02x\n", x, inb(wc->ioaddr+x));
	   delay(drv_usectohz(50000));
	}

	for(x=0; x<0x48; x+=2)
	{
	   cmn_err(CE_CONT, "Conf[%02x] = %04x\n", x, pci_config_get16(wc->pci_conf_handle, x));
	   delay(drv_usectohz(50000));
	}
#endif
...
static int wcfxo_set_daa_mode(struct wcfxo *wc)
{
	...
	drv_usecwait((1 + (ZT_CHUNKSIZE * HZ) / 800)*10000);

	cmn_err(CE_CONT, "wcfxo: DAA mode is '%s'\n", fxo_modes[opermode].name);
	return 0;
}
...
//	schedule_timeout(10);
	...
	mutex_exit(&wc->devlock);
	drv_usecwait((1 + 0.1 + (ZT_CHUNKSIZE * HZ) / 800)*10000);
	...
	if (quiet) {
		cmn_err(CE_CONT, "wcfxo: Attenuating transmit signal for quiet operation\n");
		reg15 |= (quiet & 0x3) << 4;
	}
	if (boost) {
		cmn_err(CE_CONT, "wcfxo: Boosting receive signal\n");
		reg15 |= (boost & 0x3);
	}
...
static int wcfxo_init_one(struct wcfxo *wc)
{
	int res;
	struct wcfxo_desc *d = (struct wcfxo_desc *)wc->driver_data;
	int x, size;
	size_t rlen;
	...
	for (x=0;x<WC_MAX_IFACES;x++)
		if (!ifaces[x]) break;
	if (x >= WC_MAX_IFACES) {
		cmn_err(CE_CONT, "Too many interfaces\n");
		return EIO;
	}
...
cmn_err(CE_CONT, "x = %d\n", x);
		...
		if (wc) {
			ifaces[x] = wc;
			if (ddi_regs_map_setup(wc->dip, 1, &wc->ioaddr, 0, 255,
				&dev_attr, &wc->devhandle))
				return DDI_FAILURE;

cmn_err(CE_CONT, "done ddi_regs_map_setup, ioaddr = %lx\n", wc->ioaddr);
...
cmn_err(CE_CONT, "about to ddi_dma_alloc_handle\n");
			/* Get a handle for doing DMA stuff */
		        if (ddi_dma_alloc_handle(wc->dip, 
			    &dma_attr,
       			    DDI_DMA_SLEEP, 
			    NULL, 
			...
			cmn_err(CE_CONT, "Asked for %d bytes, got %d bytes DMA space\n", size, rlen);
			...
			cmn_err(CE_CONT, "DMA cookie: addr = %lx, size = %d\n", cookie.dmac_address, cookie.dmac_size);
			...
			if (wcfxo_initialize(wc)) {
				cmn_err(CE_CONT, "wcfxo: Unable to intialize modem\n");
                		(void) ddi_dma_mem_free(&wc->dma_acc_handle);
                		ddi_dma_free_handle(&wc->dma_handle);
				return DDI_FAILURE;
			}
			...
			cmn_err(CE_CONT, "Set PCI Command Reg to %lx\n",comm);

			if (ddi_add_intr(wc->dip, 0, &wc->iblock, NULL, wcfxo_interrupt, (caddr_t)wc)) {
				cmn_err(CE_CONT, "wcfxo: Unable to request IRQ\n");
                		(void) ddi_dma_mem_free(&wc->dma_acc_handle);
                		ddi_dma_free_handle(&wc->dma_handle);
				return DDI_FAILURE;
			}

			mutex_init(&wc->devlock, NULL, MUTEX_DRIVER, (void *)&wc->iblock);

			wcfxo_hardware_init(wc);
			/* Enable interrupts */
			wcfxo_enable_interrupts(wc);
			/* Initialize Write/Buffers to all blank data */
			bzero((void *)wc->writechunk,ZT_MAX_CHUNKSIZE * 2 * 2 * 2 * 4);
			/* Start DMA */
			wcfxo_start_dma(wc);

			/* Initialize DAA (after it's started) */
			if (wcfxo_init_daa(wc)) {
				cmn_err(CE_CONT, "Failed to initailize DAA, giving up...\n");
				wcfxo_stop_dma(wc);
				wcfxo_disable_interrupts(wc);
				zt_unregister(&wc->span);

				ddi_remove_intr(wc->dip, 0, wc->iblock);
			...
			cmn_err(CE_CONT, "Found a Wildcard FXO: %s\n", wc->variety);
			res = 0;
		} else
			res = ENOMEM;
	return res;
...
static void wcfxo_remove_one(struct wcfxo *wc)
{
	if (wc) {
...
static int wcfxo_getinfo(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
                    void **result);
static int wcfxo_attach(dev_info_t *dip, ddi_attach_cmd_t cmd);
static int wcfxo_detach(dev_info_t *dip, ddi_detach_cmd_t cmd);

/* "char/block operations" OS structure */
static struct cb_ops    wcfxo_cb_ops = {
    nulldev,                    /* open() */
    nulldev,                    /* close() */
    nodev,                      /* strategy()           */
    nodev,                      /* print routine        */
...
static struct dev_ops wcfxo_ops = {
    DEVO_REV,                   /* devo_rev */
    0,                          /* devo_refcnt */
    wcfxo_getinfo,              /* devo_getinfo */
    nulldev,                    /* devo_identify */
    nulldev,                    /* devo_probe */
    wcfxo_attach,               /* devo_attach */
    wcfxo_detach,               /* devo_detach */
    nodev,                      /* devo_reset */
    &wcfxo_cb_ops,              /* devo_cb_ops */
...
int _init(void)
{
  ...
  ret = ddi_soft_state_init(&wcfxostatep, sizeof(struct wcfxo), WC_MAX_IFACES);
  if (ret != 0)
  {
    return ret;
  }
  ...
  if ((ret = mod_install(&modlinkage)) != 0) {
    cmn_err(CE_CONT, "wcfxo: _init FAILED");
    return DDI_FAILURE;
  }

  cmn_err(CE_CONT, "wcfxo: _init SUCCESS");
  return DDI_SUCCESS;
}
...
int _fini(void)
{
    ...
    if ((ret = mod_remove(&modlinkage)) == 0) {
        ddi_soft_state_fini(&wcfxostatep);
    }
    cmn_err(CE_CONT, "wcfxo: _fini %s", ret == DDI_SUCCESS ? "SUCCESS" : "FAILURE");
    return ret;
}

static int wcfxo_getinfo(dev_info_t *dip, ddi_info_cmd_t infocmd,
              void *arg, void **result)
{
  ...
  switch (infocmd)
  {
    case DDI_INFO_DEVT2DEVINFO:
      ...
      state = ddi_get_soft_state(wcfxostatep, instance);
      if (state != NULL)
      {
        *result = state->dip;
        error = DDI_SUCCESS;
...
static int wcfxo_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
  ...
  cmn_err(CE_CONT, "wcfxo_attach(cmd = %d)\n", cmd);

  switch (cmd) {
  case DDI_RESUME:
    cmn_err(CE_CONT, "wcfxo: Ignoring attach_RESUME");
    return DDI_FAILURE;
  case DDI_PM_RESUME:
    cmn_err(CE_CONT, "wcfxo: Ignoring attach_PM_RESUME");
    return DDI_FAILURE;
  case DDI_ATTACH:
    break;
  default:
    cmn_err(CE_CONT, "wcfxo: unknown attach command %d", cmd);
    return DDI_FAILURE;
  }

  instance = ddi_get_instance(dip);
  cmn_err(CE_CONT, "wcfxo: attach for instance %d", instance);

  /* Allocate some memory for this instance */
  if (ddi_soft_state_zalloc(wcfxostatep, instance) != DDI_SUCCESS)
  {
    cmn_err(CE_CONT, "wcfxo%d: Failed to alloc soft state", instance);
    return DDI_FAILURE;
  }
  ...
  if (state == NULL)
  {
    cmn_err(CE_CONT, "wcfxo%d: attach, failed to get soft state", instance);
    ddi_soft_state_free(wcfxostatep, instance);
    return DDI_FAILURE;
  }

  state->dip = dip;
  state->driver_data = &generic;

  pci_config_setup(state->dip, &state->pci_conf_handle);

  if (wcfxo_init_one(state) != DDI_SUCCESS)
  {
    cmn_err(CE_CONT, "wcfxo%d: Failed to initialise device", instance);
    ddi_soft_state_free(wcfxostatep, instance);
    return DDI_FAILURE;
  }

  cmn_err(CE_CONT, "wcfxo: attach successful!");
  return DDI_SUCCESS;
}
...
static int wcfxo_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
    ...
    instance = ddi_get_instance(dip);
    cmn_err(CE_CONT, "wcfxo%d: detach", instance);

    state = ddi_get_soft_state(wcfxostatep, instance);
    bzero(&state, sizeof(struct wcfxo));
    if (state == NULL) {
        cmn_err(CE_CONT, "wcfxo%d: detach, failed to get soft state", instance);
        return DDI_FAILURE;
    }

    wcfxo_remove_one(state);
